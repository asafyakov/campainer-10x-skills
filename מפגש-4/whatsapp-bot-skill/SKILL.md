...
## Step 2 — Find Claude binary

No questions to the user. Auto-detect Claude binary and set `$WORKDIR = ~`.
...
for p in \
  ...
  "$HOME/Library/Application Support/Claude/claude-code/$(ls "$HOME/Library/Application Support/Claude/claude-code/" 2>/dev/null | sort -V | tail -1)/claude.app/Contents/MacOS/claude"; do
  if [ -x "$p" ]; then CLAUDE_BIN="$p"; break; fi
done
echo "claude: ${CLAUDE_BIN:-NOT FOUND}"
```

- If found → store as `$CLAUDE_BIN`, add to config.json as `"claudeBin": "$CLAUDE_BIN"`
- If not found → run `npm install --ignore-scripts -g @anthropic-ai/claude-code`, then re-check

`$WORKDIR` is always `~` (macOS/Linux) or `$env:USERPROFILE` (Windows). Never ask the user.
...
Replace `<CLAUDE_BIN>` with the path found in Step 2. If Claude was installed via npm globally and found in PATH, omit the `claudeBin` field entirely.
